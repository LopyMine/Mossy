...
But we will focus on the second way. In this way you will need to copy files `stonecutter.gradle`, `settings.gradle`, `gradle.properties`, `build.gradle`, `LICENSE`, `src/main/resources/fabric.mod.json` and optional `.gitignore`. 
...
Also your game folder is now splitted to `runs/client` and `runs/server`, boop.
